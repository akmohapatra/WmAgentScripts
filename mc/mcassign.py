...
def main():
	...
	for w in list:
		...
		if reqinfo[w]['type'] == 'MonteCarloFromGEN':
			#print "Set MonteCarloFromGEN splitting"
			if reqinfo[w]['campaign'] in campaignconfig.keys() and 'lumisperjob' in campaignconfig[reqinfo[w]['campaign']].keys(): 
				lumis_per_job = campaignconfig[reqinfo[w]['campaign']]['lumisperjob']
			else:
				lumis_per_job = int(min( jobh*3600/(reqinfo[w]['timeev']) , 500000)/reqinfo[w]['inputdataset']['evperlumi'])
			if lumis_per_job < 1:
				lumis_per_job = 1
			if lumis_per_job > 10:
				lumis_per_job = 10
			params = {"requestName":w,"splittingTask" : '/'+w+"/MonteCarloFromGEN", "splittingAlgo":"LumiBased", "lumis_per_job":lumis_per_job, "timeout":"", "include_parents":"False", "files_per_job":"",'halt_job_on_file_boundaries':'True'}
			setParam(url,w,params,options.debug)

		elif reqinfo[w]['type'] == 'MonteCarlo' and reqinfo[w]['outputtier'] == 'GEN' and reqinfo[w]['lheInputFiles'] == False:	# full GEN
			#print "Set full GEN splitting "
			events_per_job = int(jobh*3600/(reqinfo[w]['timeev']))
			#events_per_job = int(max(jobh*3600/(reqinfo[w]['timeev']),10000))
			#events_per_job = 100000
			job_duration = events_per_job * reqinfo[w]['timeev'] / 3600
			#print "Job duration = %sh" % job_duration
			events_output = events_per_job * reqinfo[w]['filtereff']
			#print "Events output per job = %s" % events_output
			if job_duration > 37:
				print "duration exceeds 36h, special queue assignment needed "
				sys.exit(1)
			events_per_lumi = int(100/reqinfo[w]['filtereff'])
			params = {"requestName":w,"splittingTask" : '/'+w+"/Production", "splittingAlgo":"EventBased", "events_per_job":str(events_per_job), "events_per_lumi":str(events_per_lumi), "timeout":""}
			setParam(url,w,params,options.debug)

		elif reqinfo[w]['type'] == 'MonteCarlo' and reqinfo[w]['outputtier'] == 'GEN' and reqinfo[w]['lheInputFiles'] == True:	# full GEN
			#print "Set step0 lhe->GEN splitting "
			events_per_job = 200000
			events_per_lumi = 100
			params = {"requestName":w,"splittingTask" : '/'+w+"/Production", "splittingAlgo":"EventBased", "events_per_job":events_per_job, "events_per_lumi":events_per_lumi}
			setParam(url,w,params,options.debug)

		elif reqinfo[w]['type'] == 'MonteCarlo' and reqinfo[w]['outputtier'] == 'GEN-SIM': #regular MonteCarlo 
			if reqinfo[w]['campaign'] in campaignconfig.keys() and 'events_per_lumi' in campaignconfig[reqinfo[w]['campaign']].keys(): 
				...
				base_events_per_lumi = 100
			#print "Set MonteCarlo splitting "
			events_per_job = int( jobh*3600/(reqinfo[w]['timeev']))
			#events_per_lumi = int(min( base_events_per_lumi/reqinfo[w]['filtereff'] , 100000))
			events_per_lumi = int(base_events_per_lumi/reqinfo[w]['filtereff'])
			job_duration = events_per_job * reqinfo[w]['timeev'] / 3600
			#print "Job duration = %sh" % math.ceil(job_duration)
			events_output = events_per_job * reqinfo[w]['filtereff']
			#print "Events output per job = %s" % events_output
			params = {"requestName":w,"splittingTask" : '/'+w+"/Production", "splittingAlgo":"EventBased", "events_per_job":events_per_job, "events_per_lumi":events_per_lumi}
			setParam(url,w,params,options.debug)
